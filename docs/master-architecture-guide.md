...
**Slug validation**: `readAllProjects()` (`src/lib/projects.ts`) now rejects any filename that doesn't produce a lowercase, hyphen-separated slug (`assertValidSlug`) — e.g. `My Project!.mdx` throws instead of silently becoming the URL `/work/My Project!`. Nothing enforced this before; both real slugs at the time already satisfied it.
