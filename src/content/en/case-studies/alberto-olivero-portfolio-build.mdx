...
challenge: The site needed the same case-study architecture I use for my own portfolio, adapted for a friend's design work — but the build had diverged into an unused MDX rendering path and a security-flagged dependency version nobody had gone back to reconcile.
outcome: Delivered a working Next.js portfolio reusing my own site's architecture, then a package-manager migration that surfaced and removed the dead MDX path and its dependencies, plus a documented performance and code-quality audit with findings ready to implement.
year: "2026"
